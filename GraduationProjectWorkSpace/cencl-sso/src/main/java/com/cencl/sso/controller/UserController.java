...
public class UserController {
	...
	@ResponseBody
	public CenclResult userLogin(String username, String password,HttpServletRequest request, HttpServletResponse response) {
		try {
			CenclResult result = userService.userLogin(username, password, request, response);
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			return CenclResult.build(500, ExceptionUtil.getStackTrace(e));
	...
	public Object logout(@PathVariable String token, String callback){
		try {
			CenclResult result = userService.logout(token);
			//页面是否包含回调函数
